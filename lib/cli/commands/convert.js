...
function builder(yargs) {
  return yargs.option('credit', { default: 'Income:Crypto:MN' }).option('debit', { default: 'Assets:Crypto:Wallets:{CURRENCY}' }).option('descending', { type: 'boolean', desc: 'Sort in descending date order', default: false }).option('byDay', { type: 'boolean', desc: 'Bucket similar transactions by day', default: false }).option('start', { type: 'string', desc: 'Starting date', default: false }).option('currency', { type: 'string', desc: 'Which symbol for this conversion. EX: BTC' }).option('merge', { type: 'string', desc: 'Merge with existing YAML file' }).positional('filename', { type: 'string', desc: 'CSV file to read' });
}
