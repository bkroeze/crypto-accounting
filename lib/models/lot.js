...
function makeCreditObjects(wrappers) {
  return wrappers.map(wrapper => _extends({}, wrapper.credit.toObject({ shallow: true }), {
    applied: wrapper.applied.toFixed(8)
  }));
}

function makeDebitObjects(wrappers) {
  return wrappers.map(wrapper => _extends({}, wrapper.debit.toObject({ shallow: true }), {
    applied: wrapper.applied.toFixed(8)
  }));
}
...
class Lot {
  ...
  getCapitalGains(pricehistory, account, fiat, transCurrencies = ['BTC', 'ETH'], within = null) {
    ...
    return this.credits.map(creditWrapper => {
      ...
      const salePrice = Lot.getSalePriceEach(credit, pricehistory, fiat, transCurrencies, within);
  ...
  getUnrealizedGains(utc, pricehistory, account, currency, transCurrencies = ['BTC', 'ETH'], within = null) {
    ...
    if (remaining.gt(BIG_0)) {
      const purchasePrice = this.getPurchasePriceEach(pricehistory, currency, transCurrencies, within);
      const translation = pricehistory.findPrice(utc, this.currency, currency, transCurrencies, within);
