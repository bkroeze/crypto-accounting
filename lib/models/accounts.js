...
class Accounts {
  ...
  get(key) {
    ...
    if (!val) {
      throw makeError(ReferenceError, ERRORS.NOT_FOUND, path);
    }
    return val;
  }
  ...
  getLots(currencies, force, lifo) {
    ...
    if (force || this.lots.length === 0) {
      ...
      this.asList().filter(Account.isNotVirtualAccount).forEach(a => {
        a.getEntries(CREDIT).filter(isTrade).forEach(applyCreditToLots);
      });
      this.lots = lots;
    }
    return this.lots;
