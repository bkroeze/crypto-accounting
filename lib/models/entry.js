...
function tokenizeShortcut(shortcut, leadingSymbolMap = SYMBOL_MAP) {
  ...
  cleaned = utils.splitAndTrim(cleaned).map(fixLeadingSymbol).join(' ');
  ...
  if (tokens.length < 2) {
    ...
    throw makeError(TypeError, ERRORS.INVALID_SHORTCUT, `Invalid shortcut (need 2 parts): ${shortcut}`);
  }
...
class Entry {
  ...
  applyShortcut(shortcut, leadingSymbolMap = SYMBOL_MAP) {
    ...
    if (tokens.length > 3) {
      throw makeError(TypeError, ERRORS.INVALID_SHORTCUT, `Invalid shortcut (unknown extra fields): ${shortcut}`);
    }
  ...
  getFullShortcut(transaction = this) {
    ...
    if (this.type === DEBIT && this.isTrade()) {
      parts.push('@');
      this.pair.shortcut.split(' ').slice(0, 2).forEach(part => {
        parts.push(part);
      });
