...
class Account {
  ...
  constructor(props = {}) {
    ...
    if (!this.path) {
      ...
      throw makeError(TypeError, ERRORS.MISSING_PARAMETER, 'Invalid Account, must have a path');
    }
    if (this.parent) {
  ...
  createBalancingEntries(balancingAccount) {
    try {
      ...
      if (R.is(ReferenceError, e)) {
        console.error(e);
        throw makeError(ReferenceError, ERRORS.MISSING_ACCOUNT, `Cannot find balancing account ${balancingAccount}`);
      }
      throw e;
    }
    return this;
  ...
  getAccount(key) {
    ...
    if (!child) {
      throw makeError(ReferenceError, ERRORS.MISSING_ACCOUNT, `Account Not Found: ${this.path}:${nextChild}`);
    }
    if (path.length > 0) {
