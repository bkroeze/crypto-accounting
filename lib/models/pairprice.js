...
function parseProps(props) {
  if (RA.isString(props)) {
    ...
    if (parts.length < 3) {
      throw makeError(TypeError, ERRORS.INVALID_TERM, `Invalid price history shortcut: ${props}`);
    }
    const keyParts = parts[1].split('/');
    if (keyParts.length < 2) {
      throw makeError(TypeError, ERRORS.INVALID_TERM, `Invalid price history pair: ${parts[1]}`);
    }
...
//# sourceMappingURL=pairprice.js.map
